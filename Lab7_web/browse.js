...
function get_filtered_by_country(){
    ...
    ajax.open('POST', 'getAllDestinationsByCountry.php', true);
    // We take the choosen type for filter
    let country_name = document.getElementsByTagName("select")[0].value;
    // We convert the value type to a json string
    let json = JSON.stringify({'country_name': country_name});
    // We send the request to the server
    ajax.send(json);

    setPrevious("Country Name", country_name);
